...
export default async function AnalysisIndexPage() {
  ...
  return (
    <>
      ...
      <h2>La dada que recull gairebé tothom</h2>
      <p>
      ...
      <h2>El que no sabem</h2>
      <p>
        De {num(evidence.totalClaims)} afirmacions aplicables, {num(evidence.claims.unknown)} queden
        en desconegut: un {pct(evidence.unknownShare)}. A més, {num(evidence.claimsWithoutSources)}{' '}
        afirmacions documentades no tenen cap font; aquesta xifra hauria de ser zero.
      </p>
      <StatusStack tally={evidence.claims} unit="afirmacions" />
      <p>
        L’indicador pitjor documentat és {worstIndicator.label.toLowerCase()}, amb{' '}
        {num(worstIndicator.unknown)} fitxes de {num(worstIndicator.applicable)} sense documentar (
        {pct(worstIndicator.unknownShare)}). Amb aquest nivell de desconeixement no se’n pot
      ...
      <ul>
        <li>
          <strong>Cap xifra sense denominador.</strong> En comptes de «la majoria», hi ha{' '}
          {num(patterns.appsWithPatterns)} de {num(apps)}.
        </li>
        <li>
          <strong>Desconegut vol dir no documentat</strong>, no dolent. Els desconeguts sempre
          apareixen al costat de la xifra, i quan són tants que impedeixen concloure res, ho diem.
        </li>
        <li>
          <strong>No puntuem aquí.</strong> Les notes de cada servei són al{' '}
          <Link href="/aplicacions">directori</Link> i el càlcul, a la{' '}
          <Link href="/metodologia">metodologia</Link>. Aquestes pàgines compten pràctiques i
          expliquen com funcionen.
        </li>
      </ul>
