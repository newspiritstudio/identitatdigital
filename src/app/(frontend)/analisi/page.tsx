...
export default async function AnalysisIndexPage() {
  ...
  return (
    <>
      ...
      <p>
        {mostCollected.name} és la dada més estesa del directori: la recullen {num(mostCollected.reach)}{' '}
        de les {num(apps)} fitxes. De les que la recullen, {num(mostCollected.linkedToIdentity)} la
        vinculen a la identitat de la persona i {num(mostCollected.usedForTracking)} la fan servir
        per seguir-la fora del servei.
      </p>
      <table>
        <tbody>
          <tr>
      ...
      <p>
        ...
        un número que canvia; associada a un compte, converteix una navegació anònima en un historial
        amb nom i data. I si a més serveix per reconèixer la mateixa persona en altres llocs, el que
        es construeix ja no és l’historial d’un servei sinó el d’una persona a través de serveis que
        no ha relacionat mai entre ells.
      </p>
      <p>
      ...
      <p>
        ...
        surt sumat. {topGroups[0]?.rootName} acumula {num(topGroups[0]?.dataTypeCount ?? 0)} tipus de
        dada diferents entre les seves fitxes i {topGroups[1]?.rootName},{' '}
      ...
      <Note>
        ...
        tercers sense que la política digui amb qui, i el corpus té{' '}
        {num(graph.documentedTrackers)} rastrejadors de tercers documentats amb nom d’empresa. És el
        buit més gran del model avui, i qualsevol lectura del graf ha de comptar-hi.
      </Note>
      <p>
      ...
      <p>
        ...
        l’empresa— i en {num(patterns.appsWithoutPatterns)} hem revisat la interfície sense trobar-ne
        cap. En total n’hi ha {num(patterns.totalPatterns)} de documentats, dels quals{' '}
      ...
      <p>
        ...
        sentit es pot eliminar el compte, i {num(deletion.selfService.yes)} deixen fer-ho sense haver
        d’escriure a ningú. La mediana d’espera és de {num(deletion.medianWaitingDays ?? 0)} dies i
        la més llarga és de {num(deletion.maxWaitingDays ?? 0)}.
      </p>
      <p>
        ...
        {num(jurisdictions.appsWithTransfers)} de {num(apps)} documenten transferències de dades fora
        de la Unió Europea.
      </p>
      <Note>
      ...
      <p>
        <Link href="/analisi/sortir">Què costa marxar</Link>: esborrat, esperes, què queda desat i on
        van les dades.
      </p>
      ...
      <p>
        ...
        en desconegut: un {pct(evidence.unknownShare)}. I{' '}
        {num(evidence.claimsWithoutSources)} afirmacions documentades no tenen cap font, que és
        l’única xifra d’aquest projecte que ha de ser zero.
      </p>
      ...
      <ul>
        <li>
          ...
          {num(patterns.appsWithPatterns)} de {num(apps)}. Amb {num(apps)} fitxes, un percentatge sol
          amagar més del que explica.
        </li>
        <li>
          <strong>Desconegut no vol dir dolent.</strong> Vol dir que no ho hem documentat. Cada
          vegada que una xifra tingui desconeguts, hi són al costat; i quan són tants que no permeten
          concloure res, ho diem en comptes de concloure.
        </li>
        <li>
      ...
      <ul className="grid">
        ...
        <li className="card">
          ...
          <p className="meta">
            Els {num(patterns.totalPatterns)} patrons enganyosos documentats, per tipus, per gravetat
            i per fitxa.
          </p>
        </li>
        <li className="card">
      ...
      <p className="meta">
        ...
        registre d’incidents és una selecció editorial dels casos rellevants, no un cens: serveix per
        veure on es concentren, no per calcular-ne una taxa.
      </p>
    </>
  )
}
